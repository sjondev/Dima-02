...
public class CategoryHandler(IHttpClientFactory httpClientFactory) : ICategoryHandler
{
    ...
    public async Task<Response<Category?>> UpdateAsync(UpdateCategoryRequest request)
    {
        ...
        return await result.Content.ReadFromJsonAsync<Response<Category?>>() ?? 
               new Response<Category?>(null, 400, "Failed to modify category");
    }
    ...
    public async Task<Response<Category?>> DeleteAsync(DeleteCategoryRequest request)
    {
        ...
        return await result.Content.ReadFromJsonAsync<Response<Category?>>() ??
               new Response<Category?>(null, 400, "Failed to delete category");
    }

    public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
        => await _client.GetFromJsonAsync<Response<Category?>>($"v1/categories/{request.Id}")
            ?? new Response<Category?>(null, 400, "Failed to get category");
